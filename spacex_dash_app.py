...
def get_graph(select):
    if select == 'ALL':
        figure = px.pie(spacex_df, values='class', names='Launch Site', title='Success Rate by Launch Sites')
        return figure
    else:
        f_data = spacex_df[spacex_df['Launch Site'] == select]
        f_data = f_data['Launch Site'].groupby(spacex_df['class']).count().reset_index()
        figure = px.pie(f_data, values='Launch Site', names='class', title='Success Rate by Launch Sites')
        figure.update_layout(uniformtext_minsize=20, uniformtext_mode='hide')
        return figure
